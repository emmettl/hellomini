...
**Control Panel → Playfulness → System sounds** controls original synthesised startup, jam, feeding, cleanup, and alarm cues. Extra silliness and normal macOS output volume/mute apply. The menu bar printer reports the latest build across every saved project, independent of visible queue filters; hover for refresh errors or pause state. Click it to open Print Monitor. The thermometer reports thermal state rather than degrees. Aqua dock tiles show attention badges and optionally magnify on hover.
