...
The default look targets the early black-and-white Mac OS era. A System 7-inspired colour theme is also available. Early Aqua, specifically the Mac OS X 10.0 era with its pinstripes, remains a future direction.
...
`MiniSystem7Theme` depends on `MiniCore` and `MiniUI`, exports a `MiniThemeDefinition`, and is added by the executable to the shared registry. A future `MiniAquaTheme` can follow the same pattern. No application or desktop behavior needs an era-specific switch. Definitions provide:
...
`MiniUITests` exercises an unshipped color theme with pinstripes, gradients, rounded frames, custom renderers, and a bundled image; it also checks actual rendered pixels for each surface type. The shipped System 7 theme also tests selection persistence, rendered active/inactive chrome, and coloured/selected icons. It uses original artwork and system fonts rather than bundling Apple artwork or fonts. Aqua remains a future design.
...
Public projects can work without a token. Each saved project's **Token…** action opens an editor tied to that exact project, with explicit save and forget actions. Use GitHub Actions read access or GitLab `read_api` access. Tokens stay in this Mac's Keychain and are sent only to the selected provider's fixed HTTPS API host, never in URLs or preferences. Removing a project keeps its token; forget it first if you want it removed. A new installation starts with no projects. Unreadable or unsupported saved libraries remain untouched and block edits instead of replacing data. Self-hosted service URLs remain future work.

**Build & artifacts** opens that run's provider page for logs and artifact downloads. Job details and artifact downloads inside Hello Mini remain future work. The printer feeds imaginary paper while builds run and reports a paper jam when any visible project's latest build failed; real run states remain visible. **Control Panel → Playfulness → Print Monitor paper** and Reduce Motion stop its animation.
