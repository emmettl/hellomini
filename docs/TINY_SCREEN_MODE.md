...
Testing on the Wokyis in September 2026 confirmed that the mode is necessary and found two problems. Control Panel's theme strip embedded an AppKit scroll view with its own SwiftUI host. Under the 2× transform it drew at the enlarged size but resolved clicks at 1×, so selections drifted further from the pointer towards the right. The strip is now pure SwiftUI, with a themed scrollbar. Most other bundled apps declared minimum sizes larger than the tiny desktop, so whole windows scrolled and their toolbars disappeared. Every bundled app now reflows for narrow and short windows. A regression test checks each minimum size against tiny-screen and Purist desktops in every theme. The Aqua dock reserves 56 rather than 88 points on desktops shorter than 480 points. The published 0.4.0 build was then checked on the Wokyis and looks fine. The 1.5× comparison and the longer workflow and display-change testing remain follow-up work.
